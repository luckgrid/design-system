...
| Excluded | Why | Owner |
|---|---|---|
| `header`, `nav`, `main`, `aside`, `footer`, `section`, `article`, `search`, `menu` | landmarks and sectioning are page layout. The layouts task (DS-E01.S3.T1) kept page shells consumer-owned, so no Design System layer styles these elements; see [`layouts.md`](layouts.md) | consumer |
